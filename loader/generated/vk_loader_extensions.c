...
VKAPI_ATTR void VKAPI_CALL loader_init_device_extension_dispatch_table(struct loader_dev_dispatch_table *dev_table,
    ...
    // ---- VK_INTEL_TEST_FRAMEWORK_SUPPORT extension commands
    table->TestFrameworkDispatcherINTEL = (PFN_vkTestFrameworkDispatcherINTEL)gdpa(dev, "vkTestFrameworkDispatcherINTEL");
...
VKAPI_ATTR void* VKAPI_CALL loader_lookup_device_dispatch_table(const VkLayerDispatchTable *table, const char *name, bool* found_name) {
    ...
    *found_name = true;
    ...
    // ---- VK_INTEL_TEST_FRAMEWORK_SUPPORT extension commands
    if (!strcmp(name, "TestFrameworkDispatcherINTEL")) return (void *)table->TestFrameworkDispatcherINTEL;
...
// ---- VK_INTEL_TEST_FRAMEWORK_SUPPORT extension trampoline/terminators

VKAPI_ATTR void VKAPI_CALL TestFrameworkDispatcherINTEL(
    VkDevice                                    device,
    vkTestFrameworkDispatcherConfigurationINTEL* testFrameworkDispatcherConfiguration) {
    const VkLayerDispatchTable *disp = loader_get_dispatch(device);
    if (NULL == disp) {
        loader_log(NULL, VULKAN_LOADER_FATAL_ERROR_BIT | VULKAN_LOADER_ERROR_BIT | VULKAN_LOADER_VALIDATION_BIT, 0,
                   "vkTestFrameworkDispatcherINTEL: Invalid device "
                   "[VUID-vkTestFrameworkDispatcherINTEL-device-parameter]");
        abort(); /* Intentionally fail so user can correct issue. */
    }
    disp->TestFrameworkDispatcherINTEL(device, testFrameworkDispatcherConfiguration);
}
...
bool extension_instance_gpa(struct loader_instance *ptr_instance, const char *name, void **addr) {
    *addr = NULL;
    ...
    // ---- VK_INTEL_TEST_FRAMEWORK_SUPPORT extension commands
    if (!strcmp("vkTestFrameworkDispatcherINTEL", name)) {
        *addr = (void *)TestFrameworkDispatcherINTEL;
        return true;
    }
